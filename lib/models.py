...
class GovernanceObject(BaseModel):
    ...
    # sync dtmid gobject list with our local relational DB backend
    @classmethod
    def sync(self, dtmid):
        golist = dtmid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_dtmid(dtmid, item)
    ...
    @classmethod
    def import_gobject_from_dtmid(self, dtmid, rec):
        import decimal
        import dtmilib
        import inflection
        ...
        # shim/dtmid conversion
        object_hex = dtmilib.SHIM_deserialise_from_dtmid(object_hex)
        objects = dtmilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from dtmid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from dtmid, with every run
    ...
    def vote(self, dtmid, signal, outcome):
        import dtmilib
        ...
        output = dtmid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = dtmilib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(dtmid, signal)

    def sync_network_vote(self, dtmid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = dtmid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = DTMID_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 dtmi addr, non-multisig
            if not dtmilib.is_valid_dtmi_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Dtmi address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/DtmiDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import dtmilib
        obj_data = dtmilib.SHIM_serialise_for_dtmid(self.serialise())
    ...
    def prepare(self, dtmid):
        try:
            object_hash = dtmid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = DTMID_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not dtmilib.is_valid_dtmi_address(addr, config.network):
    ...
    def hash(self):
        import dtmilib
        return dtmilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = DTMID_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, dtmid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - dtmid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, dtmid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - dtmid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, dtmid):
        now = int(time.time())
        return (self.created_at < (now - dtmid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, dtmid):
        if self.is_expired(dtmid):
            return False

        return True

    def is_deletable(self, dtmid):
        if self.is_expired(dtmid):
            return True

...
sentinel_cfg = DtmiConfig.tokenize(sentinel_config_file)


def get_dtmi_conf():
    home = os.environ.get('HOME')

    dtmi_conf = os.path.join(home, ".dtmicore/dtmi.conf")
    if sys.platform == 'darwin':
        dtmi_conf = os.path.join(home, "Library/Application Support/DtmiCore/dtmi.conf")

    dtmi_conf = sentinel_cfg.get('dtmi_conf', dtmi_conf)

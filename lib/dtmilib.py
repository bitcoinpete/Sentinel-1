...
def is_valid_dtmi_address(address, network='mainnet'):
    ...
    dtmi_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Dtmi address
        return False

    if (address_version != dtmi_version):
        return False
...
# shims 'til we can fix the dtmid side
def SHIM_serialise_for_dtmid(sentinel_hex):
    from models import DTMID_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = DTMID_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in dtmid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # dtmid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    dtmid_hex = serialise(obj)
    return dtmid_hex


# shims 'til we can fix the dtmid side
def SHIM_deserialise_from_dtmid(dtmid_hex):
    from models import DTMID_GOVOBJ_TYPES

    # unpack
    obj = deserialise(dtmid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('dtmi.conf')

...
def has_dtmi_conf():
    ...
    valid_dtmi_conf = False

    # ensure dtmi_conf exists & readable
    #
    # if not, print a message stating that Dtmi Core must be installed and
    # configured, including JSONRPC access in dtmi.conf
    try:
        f = io.open(config.dtmi_conf)
        valid_dtmi_conf = True
    except IOError as e:
        print(e)

    return valid_dtmi_conf
...
def main():
    ...
    if not has_dtmi_conf():
        print("DtmiCore must be installed and configured, including JSONRPC access in dtmi.conf")
        sys.exit(1)

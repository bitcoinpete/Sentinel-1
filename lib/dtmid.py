...
class DtmiDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some dtmid methods
    ...
    @classmethod
    def from_dtmi_conf(self, dtmi_dot_conf):
        from dtmi_config import DtmiConfig
        config_text = DtmiConfig.slurp_config_file(dtmi_dot_conf)
        creds = DtmiConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = dtmilib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = dtmilib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()

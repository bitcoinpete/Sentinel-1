...
def test_valid_dtmi_address():
    from dtmilib import is_valid_dtmi_address

    main = valid_dtmi_address()
    test = valid_dtmi_address('testnet')

    assert is_valid_dtmi_address(main) is True
    assert is_valid_dtmi_address(main, 'mainnet') is True
    assert is_valid_dtmi_address(main, 'testnet') is False

    assert is_valid_dtmi_address(test) is False
    assert is_valid_dtmi_address(test, 'mainnet') is False
    assert is_valid_dtmi_address(test, 'testnet') is True


def test_invalid_dtmi_address():
    from dtmilib import is_valid_dtmi_address

    main = invalid_dtmi_address()
    test = invalid_dtmi_address('testnet')

    assert is_valid_dtmi_address(main) is False
    assert is_valid_dtmi_address(main, 'mainnet') is False
    assert is_valid_dtmi_address(main, 'testnet') is False

    assert is_valid_dtmi_address(test) is False
    assert is_valid_dtmi_address(test, 'mainnet') is False
    assert is_valid_dtmi_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from dtmilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % dtmilib.hashit(sb_data_hex)
